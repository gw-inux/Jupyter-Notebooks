...
#    
#    plot_model = flopy.modflow.Modflow("plot_model", version="mfnwt")
#    flopy.modflow.ModflowDis(
#        plot_model,
#        nlay=1,
#        nrow=nrow,
...
#    )
#    
#    pmv = flopy.plot.PlotMapView(model=plot_model, layer=0, ax=ax)
#    
#    im = pmv.plot_array(heads)
#    pmv.plot_grid(color="gray", linewidth=0.4)
#    
#    contours = pmv.contour_array(heads, colors="black", linewidths=0.8)
#    ax.clabel(contours, fmt="%.2f", fontsize=8)
#    
#    # Mark CHD cells at eastern boundary
#    for irow in range(nrow):
#        x = (ncol - 0.5) * delr
#        y = (nrow - irow - 0.5) * delc
#        ax.plot(
#            x,
#            y,
#            "s",
...
#    X, Y = np.meshgrid(x, y)
#    
#    c = ax.contourf(X, Y, heads[::-1, :], levels=20)
#    contours = ax.contour(X, Y, heads[::-1, :], colors="black", linewidths=0.6)
#    ax.clabel(contours, fmt="%.2f", fontsize=8)
