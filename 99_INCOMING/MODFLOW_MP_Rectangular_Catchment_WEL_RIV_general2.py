...
#        particle_offsets = [(0.5, 0.5)]
#    else:
#        n_side = int(np.ceil(np.sqrt(particles_per_cell)))
#        local_positions = np.linspace(0.25, 0.75, n_side)
#    
#        particle_offsets = []
#        for lx in local_positions:
#            for ly in local_positions:
#                particle_offsets.append((lx, ly))
#    
#        particle_offsets = particle_offsets[:particles_per_cell]
...
st.markdown("**Particle tracking / MODPATH**")

active_modpath = st.checkbox("Activate particle tracking", value=False)
...
current_model_signature = {
    ...
    "active_modpath": bool(active_modpath),
...
if st.session_state.model_done:
        ...
        if run_modpath:
            try:
                with st.spinner("Running MODPATH particle tracking..."):
                    pathlines = run_modpath_from_existing_model(
                        ws=info["workspace"],
                        modelname=info["modelname"],
                        mp_exe_name=info["mp_exe_name"],
                        tracking_direction=info["tracking_direction"],
                        particles_per_cell=info["particles_per_cell"],
    ...
    # Plot particle pathlines if available
    if st.session_state.pathlines is not None:
    
        for i, pathline in enumerate(st.session_state.pathlines):
            ...
            ax.plot(
                ...
                color = 'dimgrey',
                alpha=0.5,
                label="Pathlines" if i == 0 else None,
            )
