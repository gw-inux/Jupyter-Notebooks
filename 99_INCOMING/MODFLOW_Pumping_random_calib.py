...
st.title("Transient Pumping Model")
st.markdown("""
...
with col2:
    with st.expander('Aquifer parameter'):
        ...
        random_k = st.toggle("Use random K field", value=False)

        if random_k:
            k_factor_min = st.number_input("Minimum K multiplier", value=0.5, min_value=0.01, step=0.1)
            k_factor_max = st.number_input("Maximum K multiplier", value=2.0, min_value=0.01, step=0.1)
            random_seed = st.number_input("Random seed", value=42, min_value=0, step=1)

            rng = np.random.default_rng(int(random_seed))
            k_factor = rng.uniform(k_factor_min, k_factor_max, size=(nrow, ncol))
            k_array = k * k_factor
        else:
            k_array = np.full((nrow, ncol), k)
...
current_model_signature = {
    ...
    "obs_col": obs_col,
    "random_k": random_k,
    "k_factor_min": k_factor_min if random_k else None,
    "k_factor_max": k_factor_max if random_k else None,
...
with col_run1:
    ...
    show_measured = st.toggle("Show measured data", value=False)
    ...
    if show_measured:
        ...
        # Random but reasonable true parameters around input values
        k_true = k * rng_meas.uniform(0.5, 2.0)
        ss_true = ss * rng_meas.uniform(0.5, 2.0)

        T_true = k_true * thickness
        S_true = ss_true * thickness
        Q_true = abs(q_well)

        time_measured = np.linspace(perlen / nstp, perlen, int(nstp))

        measured_drawdown = compute_theis_drawdown(
            Q_abs=Q_true,
            T=T_true,
            S=S_true,
            r=r_obs,
        ...
        measured_head = h_ini - measured_drawdown

        # Add small measurement noise
        noise = rng_meas.normal(
            loc=0.0,
            scale=0.03 * np.max(measured_drawdown),
            size=measured_drawdown.shape,
        )

        measured_drawdown_noisy = np.maximum(measured_drawdown + noise, 0.0)
        measured_head_noisy = h_ini - measured_drawdown_noisy
...
if theis_available or measured_available or st.session_state.model_results is not None:
