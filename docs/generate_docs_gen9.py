...
# 1. Resource “Contents” table is now shown whenever a page has at least one resource
#    (previously only shown when there were multiple resources).
#
# 2. This version of the script is compatible with rendering a cleaner, more visual
#    resource title block on generated pages.
#    This is achieved by:
#    - Emitting card-style HTML markup for resource headers
#      (resource-header / resource-id / resource-title)
#    - Relying on custom CSS injected via _includes/head_custom.html
