...
def computation():
    ...
    if scatter:
        ...
        if calib == 'Regular data':
            if riv:
                ax.plot(hm,hp1_riv, 'ro', label=r'measured')
        ...
        if calib == 'Irregular data':
            if riv:
                ax.plot(hm,hp2_riv, 'ro', label=r'measured')
                me, mae, rmse = compute_statistics(hm, hp2_riv)
            else:
                ax.plot(hm,hp2, 'ro', label=r'measured')
                me, mae, rmse = compute_statistics(hm, hp2)
        if calib == 'Irregular data with noise':
            if riv:
                ax.plot(hm,hp3_riv, 'ro', label=r'measured')
                me, mae, rmse = compute_statistics(hm, hp3_riv)
            else:
                ax.plot(hm,hp3, 'ro', label=r'measured')
