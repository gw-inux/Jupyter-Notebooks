...
def page_plot2():
    ...
    # RETURN VALUES HERE FOR USE IN OTHER FUNCTIONS
    st.session_state.app_state["para"] = {
        "mine": {
            "s_U0": mine_s_U0_value,
            "s_U1": mine_s_U1_value,
            "t": mine_t_value
        },
        "town": {
            "s_U0": town_s_U0_value,
            "s_U1": town_s_U1_value,
            "t": town_t_value
        },
        "env": {
            "s_U0": env_s_U0_value,
...
def page_plot3():
    # --- Plot 3
    para = st.session_state.app_state.get("para", None)
    
    # -- ASSIGN LOCAL PARAMETERS
    mine_t_value = para["mine"]["t"]
    mine_s_U0_value = para["mine"]["s_U0"]
    mine_s_U1_value = para["mine"]["s_U1"]
    env_t_value = para["env"]["t"]
    env_s_U0_value = para["env"]["s_U0"]
    env_s_U1_value = para["env"]["s_U1"]
    town_t_value = para["town"]["t"]
    town_s_U0_value = para["town"]["s_U0"]
    town_s_U1_value = para["town"]["s_U1"]
    ...
    #s_mine = np.squeeze(s_forrs[0,np.min(np.where(t2 > plot2_params["mine"]["t"]))])
    u_mine = compute_linU(s_mine, mine_s_U0_value, mine_s_U1_value)
    #u_mine = compute_linU(s_mine, plot2_params["mine"]["s_U0"], plot2_params["mine"]["s_U1"])
...
#    s_env = np.squeeze(s_forrs[2,np.min(np.where(t2>plot2_params["environment"]["t"]))])        # env    
#    u_env = compute_linU(s_env, plot2_params["environment"]["s_U0"], plot2_params["environment"]["s_U1"])
#    s_town = np.squeeze(s_forrs[1,np.min(np.where(t2>plot2_params["town"]["t"]))])      # town          
#    u_town = compute_linU(s_town, plot2_params["town"]["s_U0"], plot2_params["town"]["s_U1"])
...
# --- Plot 4
    ...
    st.session_state.app_state["para"]["mine"]["u"] = u_mine
    st.session_state.app_state["para"]["town"]["u"] = u_town
    st.session_state.app_state["para"]["env"]["u"] = u_env
    st.session_state.app_state["para"]["Q_values"] = Q_values
