...
st.subheader('Head distribution in a bucket filled with water and sand and flowing groundwater, driven by a :blue[**specific discharge**]')
...
def bucket_flow():
    ...
    # hz = h_bottom - q (z-base_level) / K  with h_bottom = 0
    # hz = - q (z-base_level) / K 
    ...
    specific_discharge = st.session_state.specific_discharge
    ...
    pressure_head = np.where(elevation_head >= base_level, specific_discharge * (z-base_level) / K, np.nan)
    total_head = np.where(elevation_head >= base_level, pressure_head + elevation_head, np.nan)
    
    if (z_measure >= base_level):
        pressure_head_measure = specific_discharge * (z_measure-base_level) / K
    ...
    ax_bucket.plot([water_left, water_left], [0, top_level_max], color='black', linewidth=4)  # left wall
    ax_bucket.plot([water_right, water_right], [0, top_level_max], color='black', linewidth=4)  # right wall
    ax_bucket.plot([water_left, water_right], [base_level, base_level], color='grey', linewidth=2)  # bottom line
