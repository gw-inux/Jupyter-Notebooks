...
def intro_scenario1_block(bc_kind):
    ...
    with left:
        if bc_kind == "Recharge":
            ...
            K_label = st.radio(
                "**MODIFY** the :red[**hydraulic conductivity _K_**] (3 steps)",
                K_labels,
            ...
            R_label = st.radio(
                "**MODIFY** the :blue[**recharge _R_**] (3 steps)",
                list(R_map.keys()),
...
They describe whether water can **enter** or **leave** the model domain, for example, through :green[**recharge**], :violet[**rivers**], **lakes**, :blue[**wells**], or **impermeable barriers** (_as shown in the following figure_).""")
